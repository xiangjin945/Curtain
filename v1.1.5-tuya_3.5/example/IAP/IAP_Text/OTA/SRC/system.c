...
void bt_uart_write_frame(unsigned char fr_type, unsigned short len)
{
  ...
  len += PROTOCOL_HEAD;
  check_sum = get_check_sums((unsigned char *)bt_uart_tx_buf, len - 1);
...
void get_mcu_bt_mode_three(void)
{
    ...
    bt_work_state = bt_uart_rx_buf[0 + DATA_START];
...
static unsigned char data_point_handle(const unsigned char value[])
{
  ...
  index = get_dowmload_dpid_index(dp_id);//获取结构体中的命令id对应得index
...
void data_handle(unsigned short offset)
{
  ...
  // mcu_dp_enum_update(DPID_BATTERY_REMAIN, 1);
	...
	case BT_STATE_CMD:                                    //bt工作状态	
		...
		if(bt_work_state==0x01||bt_work_state==0x00)
		{
			if(bt_work_state == 0x00)						//The LED flashes for three minutes when the network is not configured
			{
				lednetworkflag = 0;	
			}
			if(bt_work_state == 0x01)						
			{
				lednetworkflag = 1;
				SET_LED(SET);				
			}
			if(bt_work_state == 0x00 && bindflag == 0x02) 	//The unbind LED flashes for two seconds
			{
				SET_LED(RESET);
...
//			}
			if(bt_uart_rx_buf[offset + DATA_START+1]==0x01)//时间格式1	获取13字节ms级unix时间+2字节时区信息
			{
				my_memcpy(current_timems_string,&bt_uart_rx_buf[offset + DATA_START+2],13);
				time_stamp_ms = my_atoll(current_timems_string);
				rtc_interrupt = time_stamp_ms/1000;
				
				VS_Timezonetime = ((unsigned short)bt_uart_rx_buf[offset + DATA_START+15]<<8) | bt_uart_rx_buf[offset + DATA_START+16];
				if(VS_Timezonetime > 1200)
				{
					VS_Timezonetime = ~(VS_Timezonetime - 1);
					IS_Timezone = FALSE;
				}
				printf("time_zone_100 time = %d \r\n",VS_Timezonetime);
				//RtcCntSign = false;
			}
...
//#endif
	...
	case TUYA_BCI_UART_COMMON_QUERY_MCU_VERSION:  
	  ...
	  break;
#endif
#ifdef TUYA_BCI_UART_COMMON_FACTOR_RESET_NOTIFY //0xA1 重置
	case TUYA_BCI_UART_COMMON_FACTOR_RESET_NOTIFY:
		...
		break;
#endif
#ifdef SUPPORT_MCU_FIRM_UPDATE/*OTA升级*/
	  case TUYA_BCI_UART_COMMON_MCU_OTA_REQUEST:
      
      for (i = APP_NEW_FW_START_ADR; i < APP_NEW_FW_END_FLAG; i+=1024)
      {
          FLASH_ErasePage(i);
      }
	  ...
	  case TUYA_BCI_UART_COMMON_MCU_OTA_FILE_OFFSET:
        USART_SendData(HT_USART0,0xEC);
        // usart0_send(bt_uart_rx_buf,12);
        total_len = bt_uart_rx_buf[offset + LENGTH_HIGH] * 0x100;
        total_len += bt_uart_rx_buf[offset + LENGTH_LOW];
        mcu_ota_proc(cmd_type,&bt_uart_rx_buf[offset + DATA_START],total_len);
        break;
