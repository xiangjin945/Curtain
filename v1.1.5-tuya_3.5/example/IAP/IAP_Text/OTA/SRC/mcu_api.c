...
void bt_uart_service(void)
{
    ...
    if ((rx_in < sizeof(bt_uart_rx_buf)) && get_queue_total_data() > 0)
    {
      if (mcu_common_uart_data_unpack(Queue_Read_Byte()))
      {
        ...
        my_memset(bt_uart_rx_buf_temp, 0, 3);
        my_memset(bt_uart_rx_buf, 0, sizeof(bt_uart_rx_buf)); //将接收缓冲区清空
        UART_RX_Count = 0;
        current_uart_rev_state_type = MCU_UART_REV_STATE_FOUND_NULL;
        uart_data_len = 0;
      }
...
void bt_protocol_init(void)
{
    ...
    test_queue_in = usart0_buf;
    test_queue_out = usart0_buf;
...
unsigned char check_sumfun(unsigned char* data, int size) 
{
  int c, b;
  int crc = data[0] << 8;
  for (c = 0; c < size; c++) {
...
typedef enum
{
  MCU_REV_STATE_FOUND_NULL,
  MCU_REV_STATE_FOUND_HEAD,
  MCU_REV_STATE_FOUND_CMD,
  MCU_REV_STATE_FOUND_LEN_H,
  MCU_REV_STATE_FOUND_LEN_L,
  MCU_REV_STATE_FOUND_DATA,
  MCU_REV_STATE_UNKOWN,
} mcu_rev_state_type_t;
#define TEST_UART_RX_DATA_LEN_MAX 256

static volatile mcu_rev_state_type_t current_rev_state_type = MCU_REV_STATE_FOUND_NULL;
static uint16_t test_uart_data_len = 0;
static volatile uint16_t test_UART_RX_Count = 0;
u8 crcflag = 0;
static bool mcu_common_test_uart_data_unpack(uint8_t data)
{
	u8 crc = 0;
  bool ret = false;
  if(data == 0x88 && current_rev_state_type != MCU_REV_STATE_FOUND_DATA)
  {
    // //notesinput");
    ms_delay_systick(100);
    current_rev_state_type = MCU_REV_STATE_FOUND_HEAD;
    Production_test_uart_rx_buf[test_UART_RX_Count] = data;
    test_UART_RX_Count++;
    return ret;
  }
  
  switch (current_rev_state_type)
  {
    case MCU_REV_STATE_FOUND_NULL:

    break;
    case MCU_REV_STATE_FOUND_HEAD:
    
      Production_test_uart_rx_buf[test_UART_RX_Count] = data;
      current_rev_state_type = MCU_REV_STATE_FOUND_LEN_H;
		  test_UART_RX_Count++;
    break;

    case MCU_REV_STATE_FOUND_LEN_H:
    
      Production_test_uart_rx_buf[test_UART_RX_Count] = data;
      test_uart_data_len = ((Production_test_uart_rx_buf[test_UART_RX_Count - 1] << 8) | Production_test_uart_rx_buf[test_UART_RX_Count]);
	//printf("test_uart_data_len %d\r\n",test_uart_data_len);
      test_UART_RX_Count++;
     
     if (test_uart_data_len > TEST_UART_RX_DATA_LEN_MAX)//数据过大时
     {
          my_memset(Production_test_uart_rx_buf, 0, sizeof(Production_test_uart_rx_buf));
          test_UART_RX_Count = 0;
          current_rev_state_type = MCU_REV_STATE_FOUND_NULL;
          test_uart_data_len = 0;
     }
     else if (test_uart_data_len > 0)
     {
         
          current_rev_state_type = MCU_REV_STATE_FOUND_LEN_L;
     }
     else
     {
          current_rev_state_type = MCU_REV_STATE_FOUND_DATA;
     }

    break;

   case MCU_REV_STATE_FOUND_LEN_L:
    
     Production_test_uart_rx_buf[test_UART_RX_Count++] = data; //DATA

     test_uart_data_len--;

     if (test_uart_data_len == 0)
     {
   ...
   case MCU_REV_STATE_FOUND_DATA:

     Production_test_uart_rx_buf[test_UART_RX_Count++] = data; //sum data
     crc = get_check_sum(Production_test_uart_rx_buf,test_UART_RX_Count-1);
     //printf(" data = %d crc = %d lenth = %d\n",data,crc,test_UART_RX_Count-1);
//	printf("data = %d\r\n",Production_test_uart_rx_buf[0]);
//   printf("data = %d\r\n",Production_test_uart_rx_buf[1]);
//   printf("data = %d\r\n",Production_test_uart_rx_buf[2]);
//   printf("data = %d\r\n",Production_test_uart_rx_buf[3]);
//   printf("data = %d\r\n",Production_test_uart_rx_buf[4]);
//   printf("data = %d\r\n",Production_test_uart_rx_buf[5]);
//   printf("test_UART_RX_Count-1 = %d\r\n",test_UART_RX_Count-1);
...
{
	u8 i,j,buffer[16]={0};
    if (mcu_common_test_uart_data_unpack(Production_test_Read_Byte()))
    {
        unsigned char cmd_type = Production_test_uart_rx_buf[3];
		//printf("cmd_type = %d\r\n",cmd_type);
		if(crcflag == 0)
		{
			printf("0");
			...
			{
			  case 1: //license
				  FLASH_ErasePage(FLASH_62K_LICESEN);

				  for(i=0,j=0;i<16;i++){
					FLASH_ProgramWordData(FLASH_62K_LICESEN+j, Production_test_uart_rx_buf[i+4]);
				  	j += 4;
				...
				break;
			  case 2: //rf bluetooth signal test
				I4_LedTwinkle();
				rf_test();
			  ...
			  case 3: //motor control
				...
				else if(Production_test_uart_rx_buf[4]==1)
				{
					motor_stop();
					close_curtain();
					printf("1");
				}else if(Production_test_uart_rx_buf[4]==2){
					motor_stop();
					open_curtain();
					printf("1");
				}
			  ...
			  case 6:
				u8 count = 0;
				for(i=0,j=0;i<16;i++){
					buffer[i] = rw(FLASH_62K_LICESEN+j);
				  	j += 4;
					if(buffer[i] == 0xFF)
					{
						count ++;
