...
void TM_CaptureInit(HT_TM_TypeDef* TMx, TM_CaptureInitTypeDef* CapInit)
{
  ...
  if (TMx == HT_SCTM0 || \
      ...
      TMx == HT_SCTM3)
  {
    Filter = (u16)((CapInit->Fsampling << 4) + CapInit->Event);
  }
  else
  {
...
void TM_PwmInputInit(HT_TM_TypeDef* TMx, TM_CaptureInitTypeDef* CapInit)
{
  ...
  _TM_CHx_Config(TMx, CapInit->Channel, CapInit->Polarity, CapInit->Selection, CapInit->Filter);

  /* Set the Input Capture Prescaler value                                                                  */
  TM_CHPSCConfig(TMx, CapInit->Channel, CapInit->Prescaler);

  /* Opposite Channel Configuration                                                                         */
  _TM_CHx_Config(TMx, OppositeChannel, OppositePol, OppositeSel, CapInit->Filter);
