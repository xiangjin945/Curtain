 ...
 * @param MCTMx: where MCTMx is the selected MCTM from the MCTM peripherals.
 * @param Update: overflow/underflow request disable control
 * @param NewState: This parameter can be ENABLE(default) or DISABLE.
 * @retval None
 ************************************************************************************************************/
void MCTM_UpdateDisable(HT_MCTM_TypeDef* MCTMx, u32 Update, ControlStatus NewState)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_MCTM(MCTMx));
  Assert_Param(IS_UPDATE_STATUS(Update));
  Assert_Param(IS_CONTROL_STATUS(NewState));
