 ...
 *     @arg ADC_TRIG_GPTMx_MTO  : where x can be 0
 *     @arg ADC_TRIG_GPTMx_CH0O : where x can be 0
 *     @arg ADC_TRIG_GPTMx_CH1O : where x can be 0
 *     @arg ADC_TRIG_GPTMx_CH2O : where x can be 0
 *     @arg ADC_TRIG_GPTMx_CH3O : where x can be 0
...
/*********************************************************************************************************//**
 * @brief Configure the ADC1 regular trigger source same as the ADC0 with 0 to 255 clocks delay.
 * @param ADC_TrigDelay: Number of delay clocks.
     This parameter must be between 0x00 to 0xFF.
 * @retval None
 ************************************************************************************************************/
void ADC1_RegularTrigDelayConfig(u32 ADC_TrigDelay)
{
  /* Check the parameters                                                                                   */
  Assert_Param(IS_ADC_TRIG_DELAY(ADC_TrigDelay));

  HT_ADC1->CFGR = (ADC_TrigDelay << 8) | 0x40 | 0x01;
}

/*********************************************************************************************************//**
 * @brief Configure the ADC1 high priority trigger source same as the ADC0 with 0 to 255 clocks delay.
 * @param ADC_TrigDelay: Number of delay clocks.
     This parameter must be between 0x00 to 0xFF.
 * @retval None
 ************************************************************************************************************/
void ADC1_HPTrigDelayConfig(u32 ADC_TrigDelay)
{
  ...
  HT_ADC1->CFGR = (ADC_TrigDelay << 16) | 0x40 | 0x03;
}
/**
  * @}
  */
